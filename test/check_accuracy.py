...
def main():
    ...
    parser.add_argument("--filter-fillers", action="store_true", help="过滤语气词（如\"嗯\"、\"啊\"、\"呢\"等），不将其计入CER计算")
    ...
    # 如果启用了语气词过滤
    if filter_fillers:
        print("注意: 启用语气词过滤，\"嗯\"、\"啊\"、\"呢\"等语气词将不计入字准确率计算")
    ...
    if result:
        ...
        print(f"过滤语气词: {'是' if filter_fillers else '否'}")
